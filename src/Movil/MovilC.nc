...
implementation {
  ...
  */
  ...
  /**
  *   Comprueba si la posición actual del nodo coincide con alguna de las posiciones designadas para las plazas de aparcamiento
  *   Se utiliza una constante de error para obtener un cuadrado a partir del punto que marca una plaza
  */
  void am_i_parked(uint16_t movilXr, uint16_t movilYr) {
  ...
  */
  bool getAssignedSlot (uint8_t slots, nx_uint8_t* slotsOwners, uint8_t* assignedSlot) {
    ...
    // Recorrer el vector de propietarios de slots de la trama TDMA
    for (slotId = 0 ; slotId<slots ; slotId++) {
      if (slotsOwners[slotId] == nodeID) {
        hasAssignedSlot = TRUE;   // Afirmar que se tiene un slot
        *assignedSlot = slotId;   // Y guardar el slot asignado en la variable pasada por referencia
      }
    }
